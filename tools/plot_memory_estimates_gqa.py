...
def bytes_convert(n: int) -> str:
    """
    Convert bytes to gigabytes as a numeric string.

    Args:
        n: Number of bytes to convert.

    Returns:
        String representing the value in gigabytes (e.g., "1.23").
    """
    gb = n / (1000 ** 3)
    return f"{gb:.2f}"


def savings_percent(total_mha: int, total_gqa: int) -> float:
    """
    Calculate percentage memory savings of GQA compared to MHA.

    Args:
        total_mha: Total KV-cache bytes for Multi-Head Attention.
        total_gqa: Total KV-cache bytes for Grouped Query Attention.

    Returns:
        Percentage savings (0-100) achieved by using GQA instead of MHA.
    """
    return (1.0 - (total_gqa / total_mha)) * 100.0


def plot_abs_kv_vs_context_multi_groups() -> None:
    """
    Generate plot comparing KV-cache memory usage across context lengths.

    Creates a logarithmic plot showing total KV-cache memory requirements for MHA and GQA with different n_kv_groups
    settings across various context lengths. Saves output to 'kv_bytes_vs_context_length.pdf'.
    """
    n_heads = 24
    emb_dim = 2048
    n_layers = 48
