"""
Test suite for remaining PyTorch-based attention modules.
...
class TestMHAPyTorchFlexAttention:
    ...
    def test_output_shape_flex_attention(self, sample_inputs):
        ...
        # Skip test if flex_attention is not available
        try:
            mha = MHAPyTorchFlexAttention(d_in, d_out, num_heads, context_length, dropout)
            output = mha(batch)
        except (ImportError, AttributeError, TypeError):
            pytest.skip("PyTorch flex_attention not available or not functional in this PyTorch version")
